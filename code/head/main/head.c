...
static void
gatts_profile_a_event_handler(esp_gatts_cb_event_t event, esp_gatt_if_t gatts_if, esp_ble_gatts_cb_param_t *param) {
    static char *log_message = "";
    switch (event) {
        case ESP_GATTS_REG_EVT:
            ESP_LOGI(GATTS_TAG, "GATT server register, status %d, app_id %d, gatts_if %d", param->reg.status,
        ...
        case ESP_GATTS_READ_EVT: {
            ESP_LOGI(GATTS_TAG, "Characteristic read, conn_id %d, trans_id %" PRIu32 ", handle %d", param->read.conn_id,
                     param->read.trans_id, param->read.handle);
            esp_gatt_rsp_t rsp;
            memset(&rsp, 0, sizeof(esp_gatt_rsp_t));
            rsp.attr_value.handle = param->read.handle;
            rsp.attr_value.len = strlen(log_message);
            for (int i = 0; i < strlen(log_message); i++) {
                rsp.attr_value.value[i] = log_message[i];
            }
        ...
        case ESP_GATTS_WRITE_EVT: {
            ...
            if (!param->write.is_prep) {
                ...
                ESP_LOGI(GATTS_TAG, "First byte %d", param->write.value[0]);

                struct Message *message = (struct Message*) malloc(param->write.value[0]);
                memcpy(message, param->write.value, param->write.value[0]);
                if (message->msg_id == LOG) {
                    ESP_LOGI(GATTS_TAG, "Message identified as log");
                    struct Log *log = (struct Log*) message;
                    ESP_LOGI(GATTS_TAG, "Conversion to log DONE!");

                    ESP_LOGI(GATTS_TAG, "%d", getMessageLength(log));
                    ESP_LOG_BUFFER_HEX(GATTS_TAG, log->message, getMessageLength(log));
                    ESP_LOGI(GATTS_TAG, "Received log from segment #%d, [%d] - %.*s", log->segment_id, log->log_level, getMessageLength(log), log->message);
                }
            }
...
void app_main(void) {
    ...
    ret = esp_ble_gatts_app_register(FORWARD_CHANNEL_ID);
    if (ret) {
